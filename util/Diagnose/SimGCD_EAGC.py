...
@torch.no_grad()
def compute_soc(backbone, labeled_loader, unlabeled_loader, args, k=16):
    ...
    # old features from labeled set
    for images, labels, _ in labeled_loader:
        images = images.to(device, non_blocking=True)
        z = backbone(images).detach().float().cpu()
        feats_old.append(z)

    # novel features from unlabeled set
...
def _pick_params_for_gdc(model):
    ...
    Print fallback/info messages only once.
    """
...
class GradientCoordinator:
    ...
    def set_aga_alignment(self, labeled_mask, reference_features, scaler_scale: float = 1.0):
        ...
        if reference_features is None or labeled_mask is None:
            return
        N = int(labeled_mask.sum().item()) # number of feats
    ...
    def _fwd_hook(self, module, inputs, output):
        ...
        def _bwd_hook(grad):
            ...
            def _project(g):
                ...
                projection_weight = adaptive_projection_weight.to(dtype=g.dtype, device=g.device)  # [B]
                return g - projection_weight.unsqueeze(1) * (g @ known_subspace)

            def _apply_aga(g):
                if (delta is None) or (self._labeled_mask is None):
                    return g
                labeled_mask = self._labeled_mask.to(dtype=g.dtype, device=g.device).unsqueeze(1)  # [B,1]
...
def extract_losses_from_pstr(pstr):
    """
    Extract all sub-loss names and values from a pstr string.
    Args:
        pstr: A string formatted like
              "cls_loss: 0.5678 cluster_loss: 0.1234 ..."
    Returns:
        loss_dict: Dict mapping loss names (e.g., "cls_loss") to float values.
    """
    loss_dict = {}
    # Regex matching "loss_name: value" (supports decimal values).
...
# TRAIN
# ----------------------
def train(backbone, projector, train_loader, test_loader, unlabelled_train_loader, labeled_eval_loader, args, reference_backbone, gradient_coordinator, mu_labeled_ema):
    params_backbone = get_params_groups(backbone)
    params_projector = get_params_groups(projector)
    optimizer = SGD(
        [
            {"params": params_backbone[0]['params'], "lr": args.lr_backbone},                      # Regularized
            {"params": params_backbone[1]['params'], "lr": args.lr_backbone, "weight_decay": 0.}, # Non-regularized
            {"params": params_projector[0]['params'], "lr": args.lr},                              # Regularized
            {"params": params_projector[1]['params'], "lr": args.lr, "weight_decay": 0.},         # Non-regularized
        ],
    ...
    labeled_energy_ratio_running = mu_labeled_ema
    mu_momentum = 0.95
    eps_mu = 1e-6
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                #------------------------------Conceptor------------------------------#
                with torch.no_grad():
                    feat_backbone_pre = reference_backbone(images).detach()
                
                scale_val = float(fp16_scaler.get_scale()) if fp16_scaler is not None else 1.0
                gradient_coordinator.set_aga_alignment(mask_lab_2v, feat_backbone_pre, scaler_scale=scale_val)
                #------------------------------Conceptor------------------------------#

                feats = backbone(images)
                # z = projector[0](feats)
                # student_proj, student_out = projector[1](z)
                ...
                if (batch_idx % args.gdc_every == 0) and mask_lab.any():
                    student_for_gdc = nn.Sequential(backbone, projector)
...
if __name__ == "__main__":
    ...
    # Reference model
    parser.add_argument('--reference_epochs', default=100, type=int, help='Number of pretraining epochs')
    parser.add_argument('--reference_batch_size', default=128, type=int)
    parser.add_argument('--reference_lr', type=float, default=0.1, help='Learning rate for pretraining')
    parser.add_argument('--reference_momentum', type=float, default=0.9, help='Momentum for pretraining')
    parser.add_argument('--reference_weight_decay', type=float, default=1e-4, help='Weight decay for pretraining')
    parser.add_argument('--reference_print_freq', dest='print_freq_reference', default=10, type=int, help='Print frequency for reference model training')
    ...
    parser.add_argument('--aperture', type=float, default=4.0)   # Aperture alpha for conceptor estimation.
    parser.add_argument('--eep_weight',   type=float, default=0.3)   # Tau: projection strength for gradient adjustment.
    ...
    # pretrain_transform = ContrastiveLearningViewGenerator(base_transform=pretrain_transform, n_views=args.n_views)
    # --------------------
    # DATASETS
    # --------------------
    train_dataset, test_dataset, unlabelled_train_examples_test, datasets, labelled_dataset = get_datasets(args.dataset_name,
                                                                                         ...
                                                                                         args,
                                                                                         labeledset=True)
    # labelled_dataset.transform = pretrain_transform
    ...
    mu_labeled_ema = compute_labeled_energy_reference(reference_backbone, labeled_eval_loader, known_subspace)
    #------------------------------Conceptor------------------------------#

    # train(backbone, projector, train_loader, None, test_loader_unlabelled, args, reference_backbone, gradient_coordinator, mu_labeled_ema)
    train(backbone, projector, train_loader, None, test_loader_unlabelled, labeled_eval_loader, args, reference_backbone, gradient_coordinator, mu_labeled_ema)
