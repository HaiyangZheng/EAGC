...
def _pick_params_for_gdc(model):
    """
    固定只取 backbone 最后一个可训练 block 的参数来计算 GDC。
    约定 model = nn.Sequential(backbone, projector)
    只在第一次调用时打印一次。
    """
...
def extract_losses_from_pstr(pstr):
    """
    从 pstr 字符串中提取所有子损失的名称和数值
    参数: pstr - 格式如 "cls_loss: 0.5678 cluster_loss: 0.1234 ..."
    返回: loss_dict - 字典，键为损失名称（如 "cls_loss"），值为损失数值（float）
    """
    loss_dict = {}
    # 正则表达式：匹配 "损失名: 数值" 格式（支持数值带小数点）
...
def train(backbone, projector, train_loader, test_loader, unlabelled_train_loader,
          labeled_eval_loader, args, reference_backbone, gradient_coordinator, labeled_energy_ratio_ema):    # params_groups = get_params_groups(student)
    ...
    optimizer = SGD(
        [
            {"params": params_backbone[0]['params'], "lr": args.lr_backbone},                      # 正则化
            {"params": params_backbone[1]['params'], "lr": args.lr_backbone, "weight_decay": 0.}, # 非正则化
            {"params": params_projector[0]['params'], "lr": args.lr},                              # 正则化
            {"params": params_projector[1]['params'], "lr": args.lr, "weight_decay": 0.},         # 非正则化
        ],
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                loss_unsup = (1 - args.sup_weight) * (cluster_loss + contrastive_loss) + 2*nll_loss.mean()
...
if __name__ == "__main__":
    ...
    train(
        backbone, projector,
        train_loader, test_loader_labelled, test_loader_unlabelled,
        labeled_eval_loader,
        args, reference_backbone, gradient_coordinator, labeled_energy_ratio_ema
    )
