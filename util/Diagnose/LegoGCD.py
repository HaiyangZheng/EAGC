...
def _flatten_grads(grad_list):
    """Flatten a list of gradients into one vector, skipping None."""
    vec = []
    for g in grad_list:
        if g is None:
            continue
...
def extract_losses_from_pstr(pstr):
    """
    Extract all sub-loss names and values from the pstr string.
    Args: pstr - format like "cls_loss: 0.5678 cluster_loss: 0.1234 ..."
    Returns: a dict mapping loss names to float values.
    """
    loss_dict = {}
    # Match "loss_name: value" patterns.
    pattern = r'(\w+_loss): ([0-9]+\.[0-9]+)'
    # Find all matches.
    matches = re.findall(pattern, pstr)
    # Fill parsed key-value pairs.
...
def compute_gdc(loss_sup, loss_unsup, model):
    """
    Compute GDC using the last backbone block:
      gL  = grad(loss_sup)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                mask_old[(idx.lt(args.num_labeled_classes)) & (mask_p_true == True)] = 1  # >thr, unlabeled, correct, predicted old
                ...
                Unlabeled_true_to_old = torch.sum(mask_p_unlabeled_true_old.int()).item()  # >thr unlabeled mapped to old
                Unlabeled_true_to_novel = torch.sum(mask_p_unlabeled_true_novel.int()).item()  # >thr unlabeled mapped to novel
                                                          
                mask_p_unlabeled_wrong = (mask_lab_p == False) & (mask_all == True) &(labels_p != idx)

                Unlabeled_condiate = torch.sum(mask_condidate_unlabeled.int()).item()  # >thr unlabeled candidates
                Unlabeled_true = torch.sum(mask_p_unlabeled_true.int()).item()  # >thr unlabeled and correct
...
if __name__ == "__main__":
    ...
    # TRAIN
    # ----------------------
    train(model, train_loader, test_loader_labelled, test_loader_unlabelled, labeled_eval_loader, args)
