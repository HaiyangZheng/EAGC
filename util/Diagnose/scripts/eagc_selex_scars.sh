...
SAVE_ROOT="${SAVE_ROOT:-$ROOT_DIR/outputs/diagnose/eagc_selex_scars}"
...
python -m util.Diagnose.SelEx_EAGC \
    ...
    --grad_from_block 9 \
    --exp_name SelEx_eagc_scars_seed0 \
    --lr_backbone 0.1 \
    --lr 0.1 \
    ...
    --reference_lr 0.02 \
    --save_path "$SAVE_ROOT" \
    --reference_model_path "$SAVE_ROOT/pretrain/SelEx_block9_seed0.pth" \
    $CONFIG_VALUES \
    --aga_weight 0.7 \
    --aperture 2.0 \
    --eep_weight 0.5
