"""Open-source training entrypoint for the SelEx baseline."""
...
class GradientCoordinator:
    def __init__(self, backbone, aga_weight):
        ...
        self.known_subspace = None      # Conceptor-like known-class subspace, shape [D, D]
        self.projection_weight = None   # Per-sample projection weight, shape [B]

        # Batch-local alignment targets (overwritten every step).
        self._reference_features = None # Reference features, shape [B, D]
        self._mask_lab = None           # labeled data mask, shape->[B] bool
    ...
    # Conceptor update
    def set_conceptor(self, known_subspace):
        self.known_subspace = known_subspace.detach()

    # Per-batch projection weight
    def set_projection_weight(self, projection_weight):
        self.projection_weight = projection_weight.detach()

    # Must be called before the forward pass in the same step.
    def set_aga_alignment(self, mask_lab, reference_features, scaler_scale: float = 1.0):
        ...
        self._mask_lab = mask_lab.detach()
        self._align_weight = (float(self.aga_weight) / (N * D)) * float(scaler_scale)
        self._align_active = True

    # ---- forward hook -> attach backward hook on z ----
    def _fwd_hook(self, module, inputs, output):
        if (not torch.is_tensor(output)) or (not output.requires_grad) or (not self.enabled):
            return

        delta = None
        if self._align_active and (self._reference_features is not None):
        ...
        def _bwd_hook(grad):
            ...
            # Read the current conceptor and projection weights at backward time.
            def _project(g):
                ...
                tau = projection_weight.to(dtype=g.dtype, device=g.device)  # [B]
                return g - tau.unsqueeze(1) * (g @ known_subspace)

            def _align(g):
                if (delta is None) or (self._mask_lab is None):
                    return g
                m = self._mask_lab.to(dtype=g.dtype, device=g.device).unsqueeze(1)  # [B,1]
                return g + self._align_weight * m * delta.to(dtype=g.dtype, device=g.device)
...
def train( projection_head, model, mlp_head, train_loader, test_loader, unlabelled_train_loader, merge_train_loader, args, reference_backbone, gradient_coordinator, labeled_energy_ratio_ema):
    optimizer = SGD([
        {"params": model.parameters(), "lr": args.lr_backbone},
        {"params": list(projection_head.parameters()) + list(mlp_head.parameters()),
         "lr": args.lr},
    ],
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            if epoch%prototype_extraction_interval==0:
                uq_index, all_preds, cluster_protos_list, preds_ind_list,  metrics= \
                    extract_labeled_protos(model, mlp_head,  merge_train_loader, args=args)
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            gradient_coordinator.set_aga_alignment(mask_lab_2v, reference_features)
            features_backbone = model(images)

            features = mlp_head(features_backbone)
        ...
        with torch.no_grad():
            if (epoch+1) % train_report_interval == 0:
                ...
                all_acc, old_acc, new_acc = test_kmeans(model, mlp_head, unlabelled_train_loader, epoch=epoch, save_name='Train ACC Unlabelled',args=args)
            else:
...
def extract_labeled_protos(model, mlp_head, train_loader, args):
    model.eval()
    mlp_head.eval()
...
def test_kmeans(model, mlp_head, test_loader, epoch,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, default='vit_dino', help='Backbone model identifier')
    ...
    parser.add_argument('--reference_print_freq', default=10, type=int,
    ...
    parser.add_argument('--aga_weight', dest='aga_weight', type=float, default=1.0, help='')

    parser.add_argument('--aperture', dest='aperture', type=float, default=4.0)
    parser.add_argument('--eep_weight', dest='eep_weight', type=float, default=0.3)
    ...
    if args.base_model == 'vit_dino':
        ...
        model = load_backbone_ckpt(model, args.dino_pretrain_path, args)
        ...
        args.image_size = 224
        args.feat_dim = model.embed_dim if hasattr(model, "embed_dim") else 768
    ...
    print(f"num_unlabeled_classes {args.num_unlabeled_classes}")
    ...
    # Use a precomputed reference checkpoint if available; otherwise train one.
    ...
    if args.train_reference_model:
        ...
        # Save reference backbone weights without moving the live model.
    ...
    mlp_head = MLP_Head(input_dim=args.feat_dim, output_dim=args.feat_dim, nlayers=args.projection_head_nlayers, hidden_dim=args.hidden_dim)

    if args.backbone_type == 'dino':
        projection_head = vits_dino.__dict__['DINOHead'](
            in_dim=args.feat_dim,
            out_dim=args.mlp_out_dim,
            nlayers=args.num_mlp_layers
        )
    elif args.backbone_type == 'dinov2':
        projection_head = vits_dinov2.DINOHead(
            in_dim=args.feat_dim,
            out_dim=args.mlp_out_dim,
            nlayers=args.num_mlp_layers
        )
    ...
    train(projection_head, model, mlp_head, train_loader, test_loader_labelled, test_loader_unlabelled, merge_train_loader, args, reference_backbone, gradient_coordinator, labeled_energy_ratio_ema)
    torch.cuda.empty_cache()
    if args.report:
